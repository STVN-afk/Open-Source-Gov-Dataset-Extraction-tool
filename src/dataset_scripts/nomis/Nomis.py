...
def Employment_Conversion():
    ...
    for x in range(0,len(urls)):
        ...
        renamed_df = reshaped_df.pivot_table(
        ...
        # Cleaning Data
        renamed_df.sort_index(axis=1, inplace=True)
        renamed_df.drop(columns=["Confidence","Denominator"],inplace=True)
        renamed_df["Numerator"] = renamed_df["Numerator"].astype(int)
        renamed_df.to_csv(url_names[x])
...
def Population_Estimate_Conversion():
    ...
    output_path = helper.filePath("population_estimate.csv", ".csvs") 
    pivot_df.to_csv(output_path, index=False)
