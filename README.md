...
# Usage Instructions + Explanations
 ...
 ### TEF Outcomes 2023
  This script automates the download of CSV files from TEF's [Outcomes 2023](https://tef2023.officeforstudents.org.uk/) website. <br/>
...
This script uses the request package to retrieve the unformatted file from the site, and then formats it by removing the buffer rows at the top of the file, and saves this to a csv file.
 ...
 ### Constituency & MP 
 This script automates the extraction of MP data from the UK Parliments [Constituency & MP Dataset](https://developer.parliament.uk/) using an API Key. <br/>
 Due to the parliament API's ability to only return 20 values at a time, a for loop is used to query the API and add a list of 20 MPs to a list until none are left. When the for loop ends, this is saved to a csv file in .csvs.<br/>
 ...
 The script for trustees returns all the officers for a single trust, whether they have retired or not and their role. <br/>
 This script also sends 500 requests every 5 minutes due to API limitations.
 ...
 ## Nomis
 The scripts below are all designed to extract data from Nomis. This includes employment, ethnicity, national averages, population, population estimates and unemployment.

Each script executes a specific API Call to the Nomis Website to request a dataset, which is then cleansed and formatted before being saved into a csv file for usage.
These all use the datetime package in the helper file to get the current date to save the filename in a specific structure.

 ### Employment
 To execute script, run the following command in the terminal:
   <pre> python .\src\dataset_scripts\nomis\employment.py </pre>
 or f5 on the script in an IDE.

 ### Ethnicity
 To execute script, run the following command in the terminal:
   <pre> python .\src\dataset_scripts\nomis\ethnicities.py </pre>
 or f5 on the script in an IDE.

 ### National Averages
 To execute script, run the following command in the terminal:
   <pre> python .\src\dataset_scripts\nomis\national_averages.py </pre>
 or f5 on the script in an IDE.

### Population
 To execute script, run the following command in the terminal:
   <pre> python .\src\dataset_scripts\nomis\population.py </pre>
 or f5 on the script in an IDE.

 ### Population Estimates
 To execute script, run the following command in the terminal:
   <pre> python .\src\dataset_scripts\nomis\population_estimate.py </pre>
 or f5 on the script in an IDE.

 ### Unemployment
 To execute script, run the following command in the terminal:
   <pre> python .\src\dataset_scripts\nomis\unemployment.py </pre>
 or f5 on the script in an IDE.
